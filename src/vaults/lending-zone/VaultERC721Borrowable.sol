...
import {IERC721Vault} from "./ERC721Vault.sol";
...
contract VaultERC721Borrowable is VaultSimpleBorrowable {
    ...
    IERC721PriceOracleMock public erc721_oracle;
    ...
    constructor(
        ...
        IERC721PriceOracleMock _erc721_oracle,
    ...
    /// @notice Sets the price oracle of the vault.
    /// @param _oracle The new price oracle.
    function setErc721Oracle(IERC721PriceOracleMock _oracle) external onlyOwner {
        erc721_oracle = _oracle;
    }
        ...
        returns (uint256 liabilityAssets, uint256 liabilityValue, uint256 collateralValue)
    {
        ...
        for (uint256 i = 0; i < collaterals.length; ++i) {
            ...
            // if (cf != 0) {
                (address erc721asset, uint256 id) = IERC721Vault(collateral).getUnderlyingERC721();

                // collateralValue += (
                //     IERC721PriceOracleMock(erc721_oracle).getQuote(address(asset), erc721asset, id) * cf
                // ) / COLLATERAL_FACTOR_SCALE;
                collateralValue += (
                    IERC721PriceOracleMock(erc721_oracle).getQuote(address(asset), erc721asset, id)
                );
            // }
        }
    }
